...
export default function DhadaBook() {
    ...
    const totals = entries.reduce((acc, row) => ({
        ...
        total: acc.total + (row.total_income || 0)
                        ...
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-slate-50 text-slate-600 uppercase font-bold text-xs">
                                <tr>
                                    ...
                                    <th className="p-4 border-b text-right w-32 bg-slate-100">Total Income</th>
                                </tr>
                            </thead>
                                ...
                                {loading ? (
                                    ...
                                    entries.map((row) => (
                                        <tr key={row.id} className="hover:bg-slate-50">
                                            ...
                                            <td className="p-4 text-right font-bold bg-slate-50/50">₹ {row.total_income.toFixed(2)}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
                            {!loading && entries.length > 0 && (
                                <tfoot className="bg-slate-900 text-white font-bold">
                                    <tr>
                                        <td colSpan="2" className="p-4 text-right text-slate-400 uppercase text-xs tracking-wider">Daily Totals</td>
                                        ...
                                        <td className="p-4 text-right bg-slate-800 text-yellow-400">₹ {totals.total.toFixed(2)}</td>
                                    </tr>
                                </tfoot>
                            )}
                        </table>

...
export default function Merchants() {
    ...
    const [formData, setFormData] = useState({ name: '', mobile: '', opening_balance: 0, remark: '' });
    ...
    const addMutation = useMutation({
        ...
        onSuccess: () => { invalidate(); toast.success('Merchant added successfully!'); },
        onError: (err) => toast.error('Failed to add merchant: ' + err.message),
    });

    const updateMutation = useMutation({
        mutationFn: async ({ id, payload, remark }) => {
            const { data, error } = await supabase.from('merchants').update(payload).eq('id', id).select().single();
            if (error) throw error;
            if (remark) {
        ...
        onSuccess: () => { invalidate(); toast.success('Merchant updated successfully!'); },
        onError: (err) => toast.error('Failed to update merchant: ' + err.message),
    });

    const deleteMutation = useMutation({
        mutationFn: (id) => supabase.from('merchants').delete().eq('id', id).throwOnError(),
        onSuccess: () => { invalidate(); toast.success('Merchant deleted.'); },
    ...
    const handleSubmit = (e) => {
        ...
        setShowModal(false);
        setFormData({ name: '', mobile: '', opening_balance: 0, remark: '' });
        setEditingId(null);
    };

    const handleEdit = (m) => {
        setFormData({ name: m.name, mobile: m.mobile, opening_balance: m.opening_balance, remark: '' });
        setEditingId(m.id);
        setShowModal(true);
    };
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                {canWrite && (
                    <button
                        onClick={() => { setShowModal(true); setEditingId(null); setFormData({ name: '', mobile: '', opening_balance: 0, remark: '' }); }}
            ...
            {confirmDeleteId && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 text-center">
                        ...
                        <h3 className="text-lg font-bold text-slate-800 mb-1">Delete Merchant?</h3>
